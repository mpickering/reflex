...
data EventWriterState t f w = EventWriterState
  { _eventWriterState_nextId :: {-# UNPACK #-} !Int -- Always negative (and decreasing over time)
  , _eventWriterState_told :: ![DSum (TellId w) (Compose f (Event t))] -- In increasing order
  }

-- | A basic implementation of 'EventWriter'.
newtype EventWriterT t f w m a = EventWriterT { unEventWriterT :: StateT (EventWriterState t f w) m a }
  deriving (Functor, Applicative, Monad, MonadFix, MonadIO, MonadException, MonadAsyncException)

-- | Run a 'EventWriterT' action.
runEventWriterT :: forall t m w a. (Reflex t, Monad m, Semigroup w) => EventWriterT t Identity w m a -> m (a, Event t w)
runEventWriterT (EventWriterT a) = do
  ...
  let combineResults :: DMap (TellId w) Identity -> w
      ...
      unId :: Compose Identity f v -> f v
      unId (Compose (Identity a)) = a
  return (result, fmap combineResults $ merge $ DMap.map unId $  DMap.fromDistinctAscList $ _eventWriterState_told requests) --TODO: We can probably make this fromDistinctAscList more efficient by knowing the length in advance, but this will require exposing internals of DMap; also converting it to use a strict list might help
...
instance (Reflex t, Monad m, Semigroup w) => EventWriter t f w (EventWriterT t f w m) where
  tellEventF w = EventWriterT $ modify $ \old ->
    ...
    in EventWriterState
       ...
       , _eventWriterState_told = (tellId myId :=> Compose w) : _eventWriterState_told old
       }

instance MonadTrans (EventWriterT t f w) where
  lift = EventWriterT . lift

instance MonadSample t m => MonadSample t (EventWriterT t f w m) where
  sample = lift . sample

instance MonadHold t m => MonadHold t (EventWriterT t f w m) where
  {-# INLINABLE hold #-}
...
instance (Reflex t, Adjustable t m, MonadHold t m, Semigroup w) => Adjustable t (EventWriterT t Identity w m) where
...
instance Requester t m => Requester t (EventWriterT t f w m) where
  type Request (EventWriterT t f w m) = Request m
  type Response (EventWriterT t f w m) = Response m
...
runWithReplaceEventWriterTWith :: forall m t w a b. (Reflex t, MonadHold t m, Semigroup w)
                               => (forall a' b'. m a' -> Event t (m b') -> EventWriterT t Identity w m (a', Event t b'))
                               -> EventWriterT t Identity w m a
                               -> Event t (EventWriterT t Identity w m b)
                               -> EventWriterT t Identity w m (a, Event t b)
...
sequenceIntMapWithAdjustEventWriterTWith
  ...
  => (   (IntMap.Key -> v -> m (Event t w, v'))
      -> IntMap v
      -> Event t (p v)
      -> EventWriterT t Identity w m (IntMap (Event t w, v'), Event t (p (Event t w, v')))
     )
  -> (Incremental t (p (Event t w)) -> Event t (PatchTarget (p w)))
  -> (Event t (p (Event t w)) -> Event t (p w))
  -> (IntMap.Key -> v -> EventWriterT t Identity w m v')
  -> IntMap v
  -> Event t (p v)
  -> EventWriterT t Identity w m (IntMap v', Event t (p v'))
...
sequenceDMapWithAdjustEventWriterTWith
  ...
  => (   (forall a. k a -> v a -> m (Compose ((,) (Event t w)) v' a))
      -> DMap k v
      -> Event t (p k v)
      -> EventWriterT t Identity w m (DMap k (Compose ((,) (Event t w)) v'), Event t (p k (Compose ((,) (Event t w)) v')))
     )
  ...
  -> (forall a. k a -> v a -> EventWriterT t Identity w m (v' a))
  -> DMap k v
  -> Event t (p k v)
  -> EventWriterT t Identity w m (DMap k v', Event t (p k v'))
...
instance PerformEvent t m => PerformEvent t (EventWriterT t f w m) where
  type Performable (EventWriterT t f w m) = Performable m
  performEvent_ = lift . performEvent_
  performEvent = lift . performEvent

instance PostBuild t m => PostBuild t (EventWriterT t f w m) where
  getPostBuild = lift getPostBuild

instance TriggerEvent t m => TriggerEvent t (EventWriterT t f w m) where
  newTriggerEvent = lift newTriggerEvent
  newTriggerEventWithOnComplete = lift newTriggerEventWithOnComplete
  newEventWithLazyTriggerWithOnComplete = lift . newEventWithLazyTriggerWithOnComplete

instance MonadReader r m => MonadReader r (EventWriterT t f w m) where
  ask = lift ask
  local f (EventWriterT a) = EventWriterT $ mapStateT (local f) a
  reader = lift . reader

instance MonadRef m => MonadRef (EventWriterT t f w m) where
  type Ref (EventWriterT t f w m) = Ref m
  newRef = lift . newRef
  readRef = lift . readRef
  writeRef r = lift . writeRef r

instance MonadAtomicRef m => MonadAtomicRef (EventWriterT t f w m) where
  atomicModifyRef r = lift . atomicModifyRef r

instance MonadReflexCreateTrigger t m => MonadReflexCreateTrigger t (EventWriterT t f w m) where
  newEventWithTrigger = lift . newEventWithTrigger
  newFanEventWithTrigger f = lift $ newFanEventWithTrigger f

instance (MonadQuery t q m, Monad m) => MonadQuery t q (EventWriterT t f w m) where
  tellQueryIncremental = lift . tellQueryIncremental
  askQueryResult = lift askQueryResult
  queryIncremental = lift . queryIncremental

instance DynamicWriter t w m => DynamicWriter t w (EventWriterT t f v m) where
  tellDyn = lift . tellDyn

instance PrimMonad m => PrimMonad (EventWriterT t f w m) where
  type PrimState (EventWriterT t f w m) = PrimState m
